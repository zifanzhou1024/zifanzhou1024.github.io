...
### Reproducible one-liner verification

Run this in one command to get deterministic build checks:

```bash
docker run --rm -v "$(pwd)":/srv/jekyll -w /srv/jekyll jekyll/jekyll:latest sh -lc "bundle install && bundle exec jekyll build"
```
